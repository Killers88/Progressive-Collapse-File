 ...
 *
 */
public class internalColumnForcesAndDesignA {
	...
	public static Double maxMx(final double span, final double trib,
		...
		double deadUdl = trib * (sdl / 1000);

		double designUDL = ultUdl - deadUdl;

		double mMax = designUDL * Math.pow(span, 2) / 12;
	...
	public static String columnSizing(final double span, final double trib,
		...
		double deadAxial = windForces
				.axialInt(span, trib, sdl, live, numstorys);

		double windMoment = windForces.intMoment(colhgt, numstorys, trib, bays,
				windspeed);
